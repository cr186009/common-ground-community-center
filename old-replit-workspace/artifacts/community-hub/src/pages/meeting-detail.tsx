...
import { formatDateTimeRange, getMeetingTypeLabel, parseStoredList } from '@/lib/hub-format';

export default function MeetingDetailPage() {
  const { id } = useParams<{ id: string }>();
  const { data: meeting, isLoading, isError } = useGetMeeting(id!);
  ...
  if (isError || !meeting) {
    return (
      <div className="space-y-4 text-center py-20">
        <p className="font-serif text-2xl text-[color:var(--navy)]">Meeting not found</p>
        <Link href="/meetings" className="text-sm text-[color:var(--forest)] hover:underline">
          ← Back to meetings
        </Link>
      </div>
    );
          ...
          <div className="rounded-[1.75rem] border border-[color:var(--line)] bg-white p-6">
            ...
            <dl className="mt-5 grid gap-5 text-sm">
              <div>
                ...
                <dd className="mt-1">
                  {meeting.agendaUrl ? (
                    <a href={meeting.agendaUrl} target="_blank" rel="noreferrer" className="text-[color:var(--forest)]">Open agenda</a>
              ...
              <div>
                ...
                <dd className="mt-1">
                  {meeting.minutesUrl ? (
                    <a href={meeting.minutesUrl} target="_blank" rel="noreferrer" className="text-[color:var(--forest)]">Open minutes</a>
              ...
              <div>
                ...
                <dd className="mt-1">
                  {meeting.videoUrl ? (
                    <a href={meeting.videoUrl} target="_blank" rel="noreferrer" className="text-[color:var(--forest)]">Watch video</a>
                ...
                <div>
                  ...
                  <dd className="mt-1">
                    <a href={meeting.originalUrl || meeting.sourceUrl || '#'} target="_blank" rel="noreferrer" className="text-[color:var(--forest)]">
                      {meeting.sourceName || 'View source'}
                    </a>
                  </dd>
                </div>
              ) : null}
        ...
        <aside className="space-y-6">
          <div className="rounded-[1.75rem] border border-[color:var(--line)] bg-[color:var(--gold-soft)]/50 p-6">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {keyTopics.length > 0 ? (
                keyTopics.map((topic) => (
                  <span key={topic} className="rounded-full bg-white px-3 py-1 text-sm text-slate-700">{topic}</span>
                ))
              ) : (
          ...
          <div className="rounded-[1.75rem] border border-[color:var(--line)] bg-white p-6">
            ...
            <p className="mt-4 text-sm leading-7 text-slate-700">
              {meeting.whyResidentsCare || 'Meeting outcomes can affect local services, budgets, infrastructure, neighborhood growth, and how public resources are used.'}
            </p>
          </div>
        </aside>
      </section>
